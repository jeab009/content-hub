...
## Google/YouTube connect ใช้งานไม่ได้ผ่าน `docker compose` — 2026-08-05

ระหว่าง UAT smoke test บน Settings page พบว่า Google/YouTube ไม่มีปุ่มเชื่อมเลย ทั้งที่ backend มี route `GET /api/connected-accounts/google/authorize` + `/callback` ทำไว้สมบูรณ์ตั้งแต่ task #4 (mirror ของ Facebook ทุกจุด) — เพิ่ม `googleAuthorizeUrl()` ใน `api-client.ts` + ปุ่ม "Connect a YouTube Channel" ใน `settings/page.tsx`. ระหว่างแก้เจอบั๊กที่สอง: `STATUS_MESSAGES` hardcode ข้อความ "Facebook" ทุก status (success/cancelled/error) — ทั้งที่ backend's `handleOAuthCallback` ส่ง `message` param ที่ provider-aware อยู่แล้วสำหรับ error case (`Could not connect to Google...`) แต่ frontend ไม่เคยอ่านค่านั้นเลย แก้ให้อ่าน `searchParams.get('message')` จริง และทำ success/cancelled เป็น provider-neutral copy.

หลัง user ใส่ Google Client ID/Secret จริงแล้วทดสอบ พบบั๊กที่สามซึ่งเป็น **root cause แท้จริง**: `docker-compose.yml`'s backend service `environment` block **ไม่เคยมี `GOOGLE_*` เลยสักตัว** (มีแค่ `FACEBOOK_APP_ID`/`FACEBOOK_APP_SECRET`) — โค้ด backend ถูกต้องมาตลอดตั้งแต่ task #4 แต่ deployment path ผ่าน `docker compose up` ไม่เคยส่ง credential เข้า container เลย (`GET /api/connected-accounts/google/authorize` ตอบ `409 Google OAuth is not configured` เสมอไม่ว่า `.env` จะมีค่าอะไรก็ตาม). คำแนะนำเดิมที่บอก user ให้ใส่ credential ใน `backend/.env` ก็ผิดจุดด้วยเช่นกัน — ของจริงที่ docker-compose ใช้ interpolate คือ root `.env` (pattern เดียวกับ `FACEBOOK_APP_ID` ที่มีอยู่แล้ว)

แก้: copy ค่าจริงจาก `backend/.env` ไป root `.env` ด้วย `grep 'GOOGLE_CLIENT_(ID|SECRET)' backend/.env >> .env` (output ไม่ผ่านหน้าจอ ไม่โชว์ secret เลย) แล้วเพิ่ม `GOOGLE_CLIENT_ID`/`GOOGLE_CLIENT_SECRET`/`GOOGLE_REDIRECT_URI`/`GOOGLE_OAUTH_SCOPES` เข้า `docker-compose.yml`'s backend environment block ตาม pattern ของ Facebook เป๊ะ (blank-safe default, boot ได้ปกติแม้ไม่ตั้งค่า).
