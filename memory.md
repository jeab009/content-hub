...
- **กำลังทำ: Phase 4B frontend** — `/comments` inbox + reply composer modal + escalation banner (against frozen 4A API)
- **Not implemented**: Phase 4B frontend (in progress), 4C real model sentiment (behind SENTIMENT_IMPL=model, disabled), TikTok/LINE (Phase 5). Cron auto-sync defer (Phase 3.5 bundle) — ตอนนี้ manual Sync button
