...
- **Not implemented**: Phase 5B frontend (4-platform publish UI + manual-record modal + v2 reasoning + revenue drill-down + export buttons), 5C (live adapters + PDF), 4C model sentiment. Cron auto-sync defer (Phase 3.5 bundle) — ตอนนี้ manual Sync button
- Carry-forward (non-blocking): QC 4B 3 UX-minor, QA-OBS-2 docs sync, cron auto-sync, 401-log-noise WARN downgrade
